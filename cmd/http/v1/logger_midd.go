...
func GinLogger(appCtx app_context.AppContext) gin.HandlerFunc {
	return GinLoggerWithConfig(appCtx, GinLoggerConfig{})
}
...
func GinLoggerWithConfig(appCtx app_context.AppContext, conf GinLoggerConfig) gin.HandlerFunc {
	...
	return func(c *gin.Context) {
		...
		defer func() {
			...
			ginLogger.With(zap.Time("req_time", details.ReqTime),
				zap.String("client_ip", details.ClientIP),
				zap.String("method", details.Method),
				zap.String("path", details.Path),
				zap.String("host", details.Host),
			...
			if len(c.Errors) > 0 {
				ginLogger.With(zap.String("context_errors", c.Errors.String()))
			}
			// 判断是否打印 context keys
			if conf.EnableContextKeys {
				details.ContextKeys = c.Keys
				ginLogger.With(zap.Any("context_keys", details.ContextKeys))
			}
			// 判断是否打印请求 header
			if conf.EnableRequestHeader {
				ginLogger.With(zap.Any("request_header", details.RequestHeader))
			}
			// 判断是否打印请求 form
			if conf.EnableRequestForm {
				ginLogger.With(zap.Any("request_form", details.RequestForm))
			}
			// 判断是否打印请求 body
			if conf.EnableRequestBody {
				ginLogger.With(zap.Any("request_body", details.RequestBody))
			}
			// 判断是否打印响应 body
			if conf.EnableResponseBody {
				...
				ginLogger.With(zap.Any("response_body", details.ResponseBody))
			}

			// details logger 可以打印更多字段
			ginLogger.Named("details").With(
