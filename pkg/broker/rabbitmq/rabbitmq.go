...
func (r *RabbitMQ) Connect() (*RabbitMQ, error) {
	// if we already connected do not re-connect
	if r.conn != nil {
		return r, nil
	}
...
func (r *RabbitMQ) handleErrors(conn *amqp.Connection) {
	...
	go func() {
		for b := range conn.NotifyBlocked(make(chan amqp.Blocking)) {
			if b.Active {
				r.logger.Infof("TCP blocked: %q", b.Reason)
			} else {
				r.logger.Info("TCP unblocked")
			}
		}
	}()
}
