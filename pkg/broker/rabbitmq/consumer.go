...
func (c *Consumer) Consume(handler func(delivery amqp.Delivery)) error {
	...
	c.RabbitMQ.logger.Info("handle: deliveries channel starting")
	...
	c.RabbitMQ.logger.Info("handle: deliveries channel closed")
	c.done <- nil
	return nil
}
...
func (c *Consumer) Get(handler func(delivery amqp.Delivery)) error {
	...
	if ok {
		c.RabbitMQ.logger.Debug("Message received")
		handler(message)
	} else {
		c.RabbitMQ.logger.Debug("No message received")
	}
...
func (c *Consumer) Shutdown() error {
	...
	defer c.RabbitMQ.logger.Info("Consumer shutdown OK")
	c.RabbitMQ.logger.Info("Waiting for Consumer handler to exit")
